...
AI Writer is the research writing environment inside Paperguide. You start from a research question or an existing draft, and AI Writer produces academic writing with citations drawn from Paperguide's 200M+ paper database and your reference manager—then helps you refine it in the same editor. Every claim it writes links back to a source you can verify, so the draft stays traceable to the literature it came from.
...
- **Create new document**: start a new paper, report, essay, or research document in AI Writer.

- **Import Word document**: bring an existing `.docx` draft into Paperguide to revise, expand, or cite. There's no setup: the document opens in the editor and you can start writing immediately.\
  To get better AI recommendations afterward, open **Document Settings** and add a document description and citation sources (described below).

### 1. Choose how to start

After selecting **Create new document**, choose one of three paths:
...
- **Citation style**: the format used for in-text citations and the reference list (1,000+ styles supported). *Applies to all paths.*

- **Language**: the writing language of the document. *Applies to all paths.*

- **Number of references**: the maximum number of references AI may cite. *(For Generate Document only.)*

- **Word count**: a target length for the generated draft. AI Writer aims for the nearest range rather than an exact count, prioritizing the quality of writing for your topic within that range. *(For Generate Document only.)*
